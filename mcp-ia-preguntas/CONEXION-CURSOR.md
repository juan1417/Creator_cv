...
| `context/cv-interview.pending.template.json` | Esquema de **una ronda**: la IA copia el formato a `cv-interview.cv<id>.pending.json` con la pregunta actual (`<id>` = id del CV en la app). |
| `context/cv-interview.cv*.pending.json` | Una pregunta activa **por CV** (la web muestra y elimina al guardar). Ignorados en git salvo que quieras versionar ejemplos. |
...
5. En el chat, pide a la IA que actualice `cv-context.active.json` y el pending **del CV concreto**: `cv-interview.cv<id>.pending.json`.
...
- **Entrevista MCP**: la IA escribe `cv-interview.cv<id>.pending.json`; la app muestra la pregunta, fusiona el contexto y guarda la revisión en BD y `cv-review-cv<id>.active.md`. Opcional: `CREATOR_CV_INTERVIEW_PENDING_PATH` con `{id}` o apuntando a un directorio.
